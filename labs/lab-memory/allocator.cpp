...
{
//cout << "Create letter groups..\n";
    createLetterGroups();
//cout << "Load students..\n";
    loadStudents(studentFile);
//cout << "Create load rooms..\n";
    loadRooms(roomFile);
//cout<< "complete.\n";
}
...
void Allocator::loadStudents(const string& file)
{
    ...
    for (int i = 0; i < studentCount; i++) {
        ...
        char letter = name[0];
//cout << "For studentCount: " << i << ", letter: " << letter << endl;
...
void Allocator::loadRooms(const string& file)
{
    ...
    roomCount = fileio::getNumRooms(); //changed
...
//cout << "About to call addLetter with " << alpha[L].letter << " into " << r->name << endl;
        r->addLetter(alpha[L]);}
...
int Allocator::minSpaceRemaining()
{
    int border = 1000000;
    for (int i = 0; i < roomCount; i++){
//cout << "Room " << rooms[i].name << " has " << rooms[i].spaceRemaining() << " spots.\n";
