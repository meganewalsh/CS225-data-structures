...
{
    /* Your code goes here! */
  string w1 = word1;	  std::transform(w1.begin(), w1.end(), w1.begin(), ::toupper);
  string w2 = word2;  	  std::transform(w2.begin(), w2.end(), w2.begin(), ::toupper);

  if ((dict.find(w1) != dict.end()) && (dict.find(w2) != dict.end()) && (dict.find(w1)->second == dict.find(w2)->second))	return true;
  else																return false;
