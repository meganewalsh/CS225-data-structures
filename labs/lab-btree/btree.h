...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
  ...
  if (elements.empty() || elements[0] > val ) return 0;
  if (elements[elements.size()-1] < val) return elements.size();
  if (elements[elements.size()-1] == val) return elements.size()-1;  

  unsigned low = 0, high = elements.size()-1;
  for(;;){

     size_t midpoint = (high+low)/2;
     if (elements[midpoint] == val || ((elements[midpoint] > val) && elements[midpoint-1] < val ))
	return midpoint;
     else if (elements[midpoint] < val)
	low = midpoint + 1;
     else
	high = midpoint - 1;
  }
