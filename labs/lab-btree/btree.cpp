...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
  ...
  unsigned size = subroot->elements.size();

  //Check valid index. Key there is key we are looking for
  if (size && first_larger_idx < size && subroot->elements[first_larger_idx].key == key) //is_valid(subroot, subroot->elements, order)){
	  return subroot->elements[first_larger_idx].value;

//Otherwise, figure out which child
  //Failed, so return default
  if (subroot->is_leaf)		return V();
  //The idx is the same as first_larger_idx. Recurse with child
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
  ...
  //Insert pointer to new right node - empty
  parent->children.insert(child_itr, new_right);
  //Insert mid into parent - note median still in child
  parent->elements.insert(elem_itr, *mid_elem_itr);

  //Copy over to left and right 
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
  ...
  unsigned size = subroot->elements.size();
  //Case empty
  if (!size)   subroot->elements.insert(subroot->elements.begin(), pair);

  //Check exists (by insertion_idx)  and valid
  else if (first_larger_idx < size && subroot->elements[first_larger_idx].key == pair.key)
	return;

  //Case 1: Leaf & key doesn't exist
  if (subroot->is_leaf)
	//Insert pair into subroot
	subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair); 
       // BTreeNode* newNode = new BTreeNode(true, order);
  //Case 2: Not a leaf & key dne
  else{
	//Figure out which child and call insert
	BTreeNode* whereToInsert = subroot->children[first_larger_idx];
	insert(whereToInsert, pair);
	//Once call returns, check if child too large .. split?
	if (whereToInsert->elements.size() >= order)
