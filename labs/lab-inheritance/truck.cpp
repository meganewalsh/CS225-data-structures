...
{
Rectangle* trailer = new Rectangle(Vector2(center_.x() - TRAILER_WIDTH / 2, center_.y()),
                        TRAILER_COLOR, TRAILER_WIDTH, TRAILER_HEIGHT);
trailer->draw(canvas);

Rectangle* cabin = new Rectangle(Vector2(center_.x() + CABIN_WIDTH / 2,
                  (center_.y() + TRAILER_HEIGHT / 2) - CABIN_HEIGHT / 2),
          CABIN_COLOR, CABIN_WIDTH, CABIN_HEIGHT);
cabin->draw(canvas);

Rectangle* engine = new Rectangle(Vector2(cabin->center().x() + CABIN_WIDTH / 2 + ENGINE_WIDTH / 2,
                  (cabin->center().y() + CABIN_HEIGHT / 2) - ENGINE_HEIGHT / 2),
          ENGINE_COLOR, ENGINE_WIDTH, ENGINE_HEIGHT);
engine->draw(canvas);

Rectangle* window = new Rectangle(Vector2(cabin->center().x(), cabin->center().y() - WINDOW_HEIGHT / 2),
          WINDOW_COLOR, WINDOW_WIDTH, WINDOW_HEIGHT);
window->draw(canvas);
...
for(unsigned ctr = 0; ctr < NUM_WHEELS; ctr++){
	wheels[ctr]->draw(canvas);
}	
  }
