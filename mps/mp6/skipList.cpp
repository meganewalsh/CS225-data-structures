...
SkipList::SkipList() {
    ...
    head_->nodePointers[0].prev = NULL; //added
    tail_->nodePointers[0].next = NULL; //added
...
/**
 * Inserts a new node into the sorted order in the list, initialized with
 * the provided values.
...
void SkipList::insert(int key, HSLAPixel value) {
    ...
    // If the SkipNode exists, the key exists in our list and the value
    // for that key must
    ...
    int level = listHeight_ -1; //head_->nodePointers.size() - 1;
...
SkipNode * SkipList::findR(int key) {
    return findRHelper(key, listHeight_-1/*head_->nodePointers.size() - 1*/, head_);
}
 ...
 * @return A pointer to the node with the given key, or NULL if it could not
 * be found
 */
 ...
 * @param key The key to search for
 * @return A pointer to the node with the given key, or NULL
